...
3. Solves either the squared-distance Dirichlet model

       E(f) = 1/2 * sum_{ij in E} c_ij * d_H(f_i, f_j)^2

   or the hyperbolic mean-value balance equations, by Riemannian descent using
   the exponential map of the Poincare disk.
...
-----------------
{
  ...
  "edge_force_model": "squared_distance",   // or "hyperbolic_mean_value"
...
Example
-------
uv run hyper-morph-solve example_disk_mesh.json --output result.json
...
class HarmonicMapSolver:
    def __init__(self, data: Dict[str, object]) -> None:
        ...
        self.edge_force_model = str(data.get("edge_force_model", "squared_distance"))
        if self.edge_force_model not in {
            "squared_distance",
            "hyperbolic_mean_value",
        }:
            raise ValueError(
                "'edge_force_model' must be 'squared_distance' or "
                f"'hyperbolic_mean_value', got {self.edge_force_model!r}."
            )
    ...
    def energy_and_raw_gradient(
        ...
        for (i, j), energy_weight, (weight_i, weight_j) in zip(
            ...
            if self.edge_force_model == "hyperbolic_mean_value":
                energy += energy_weight * (math.cosh(d) - 1.0)
    ...
    def solve(self) -> Dict[str, object]:
        ...
        last_grad_norm = None
        stop_reason = "zero_iterations" if self.iterations == 0 else "max_iterations"
        ...
        return self._result(final_energy, last_grad_norm, stop_reason)
    ...
    def _result(
        ...
        return {
            ...
            "edge_force_model": self.edge_force_model,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--edge-force-model",
        choices=("squared_distance", "hyperbolic_mean_value"),
        help="Override the edge-force model.",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    if args.edge_force_model is not None:
        data["edge_force_model"] = args.edge_force_model
